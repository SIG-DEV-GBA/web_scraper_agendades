"""Insert Andalucía events from viralagenda.com (all 8 provinces)."""
...
# All Andalucía viralagenda sources
ANDALUCIA_SOURCES = [
    ("viralagenda_almeria", "Almería"),
    ("viralagenda_cadiz", "Cádiz"),
    ("viralagenda_cordoba", "Córdoba"),
    ("viralagenda_granada", "Granada"),
    ("viralagenda_huelva", "Huelva"),
    ("viralagenda_jaen", "Jaén"),
    ("viralagenda_malaga", "Málaga"),
    ("viralagenda_sevilla", "Sevilla"),
]
...
async def main(
    ...
    """Fetch, enrich and insert Andalucía events from all viralagenda sources.
    ...
    """
    print(f"\n{'='*60}")
    print(f"ANDALUCÍA VIRALAGENDA - Inserción de eventos")
    ...
    sources_to_process = ANDALUCIA_SOURCES
    if provinces:
        provinces_lower = [p.lower() for p in provinces]
        sources_to_process = [
            (sid, prov) for sid, prov in ANDALUCIA_SOURCES
    ...
    # Summary
    print(f"\n{'='*60}")
    print(f"RESULTADO TOTAL ANDALUCÍA:")
