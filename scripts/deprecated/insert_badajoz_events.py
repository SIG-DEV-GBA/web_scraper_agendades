"""Insert Badajoz (Extremadura) events using Firecrawl + Bronze scraper."""
...
SOURCE_ID = "badajoz_agenda"
CCAA = "Extremadura"
PROVINCE = "Badajoz"


async def main(limit: int = 50, dry_run: bool = False):
    """Fetch, enrich and insert Badajoz events."""
    print(f"\n{'='*60}")
    print(f"BADAJOZ AGENDA (Firecrawl + Bronze) - Insercion de eventos")
    ...
    # 1. Fetch events from listing page via Firecrawl
    print("1. Fetching events from aytobadajoz.es via Firecrawl...")
    raw_events = await adapter.fetch_events(fetch_details=True)
    print(f"   -> Found {len(raw_events)} raw events")

    if not raw_events:
        print("No events found. Check Firecrawl connection.")
        return 0
    ...
    if not events:
        print("No future events to process.")
        return 0

    # Show sample events
    print("\n   Sample events:")
    for e in events[:3]:
        title_display = e.title[:50] if e.title else "Sin titulo"
