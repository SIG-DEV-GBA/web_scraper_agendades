#!/usr/bin/env python
"""Run Viralagenda pipeline: scrape -> LLM enrich -> geocode -> insert.

Usage:
    python scripts/run_viralagenda.py --province valladolid --dry-run
    python scripts/run_viralagenda.py --province sevilla --no-dry-run
    python scripts/run_viralagenda.py --all --no-dry-run --no-llm
"""
...
from src.adapters.bronze.viralagenda import VIRALAGENDA_SOURCES, get_viralagenda_source_ids
...
async def run_viralagenda_pipeline(
    sources: list[str],
    dry_run: bool = True,
    llm_enabled: bool = True,
    images_enabled: bool = True,
    fetch_details: bool = True,  # Default True for Viralagenda (needed for full descriptions)
    limit: int | None = None,
) -> dict[str, dict]:
    """Run the Viralagenda pipeline for specified sources.
    ...
    for source_slug in sources:
        ...
        try:
            # 1. Get adapter and scrape events
            adapter_class = get_adapter(source_slug)
            if not adapter_class:
                print(f"[{source_slug}] ERROR: Adapter not found")
                results[source_slug] = {"error": "Adapter not found"}
                continue
            ...
            # 2.5. Filter out past events (start_date must be >= today)
            today = date.today()
            ...
            if llm_enabled and enricher and enricher.is_enabled and events:
                ...
                # Run LLM enrichment (Bronze tier for chaotic web sources)
                ...
                for event in events:
                    ...
                    if eid and eid in enrichments:
                        ...
                        if enrichment.price is not None and event.price is None:
                            ...
                            event.is_free = False
                        # price_info is for descriptive text, not the price itself
            ...
            if not dry_run:
                ...
                config = VIRALAGENDA_SOURCES[source_slug]
                batch = EventBatch(
...
def main():
    parser = argparse.ArgumentParser(description="Run Viralagenda scraping pipeline")
    parser.add_argument(
        "--province", "-p",
        help="Province slug to process (e.g., valladolid, sevilla, caceres)",
    )
    parser.add_argument(
        "--ccaa", "-c",
        help="Process all provinces in a CCAA (e.g., andalucia, galicia, castilla-y-leon)",
    )
    parser.add_argument(
        "--all", "-a",
        action="store_true",
        help="Process all Viralagenda sources",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--no-fetch-details",
        action="store_true",
        help="Skip fetching detail pages (faster but descriptions will be LLM-generated)",
    )
    parser.add_argument(
        "--limit", "-l",
        type=int,
    ...
    parser.add_argument(
        "--list",
        action="store_true",
        help="List all available Viralagenda sources",
    )

    args = parser.parse_args()

    # Get all source IDs
    all_source_ids = get_viralagenda_source_ids()

    # List sources
    if args.list:
        print("Available Viralagenda sources:")
        for slug in sorted(all_source_ids):
            config = VIRALAGENDA_SOURCES[slug]
            print(f"  {slug}: {config.ccaa} / {config.province}")
        return
    ...
    if args.all:
        ...
        ccaa_lower = args.ccaa.lower().replace("-", " ")
        ccaa_mapping = {
            "andalucia": "Andalucía",
            "galicia": "Galicia",
        ...
        if ccaa_name:
            sources = [
                slug for slug in all_source_ids
                if VIRALAGENDA_SOURCES[slug].ccaa == ccaa_name
            ]
        else:
            print(f"Unknown CCAA: {args.ccaa}")
            print(f"Available: {', '.join(ccaa_mapping.keys())}")
            return
    elif args.province:
        slug = f"viralagenda_{args.province.lower()}"
        if slug in all_source_ids:
            sources = [slug]
        else:
            print(f"Unknown province: {args.province}")
            print(f"Available: {', '.join(s.replace('viralagenda_', '') for s in sorted(all_source_ids))}")
            return
    else:
        parser.print_help()
        return
    ...
    # Run pipeline
    asyncio.run(run_viralagenda_pipeline(
        sources=sources,
        dry_run=dry_run,
        llm_enabled=not args.no_llm,
        images_enabled=not args.no_images,
        fetch_details=not args.no_fetch_details,  # Default True, use --no-fetch-details to disable
        limit=args.limit,
    ))
