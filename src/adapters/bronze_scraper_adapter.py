...
from src.config.sources import BronzeSourceConfig, SourceRegistry, SourceTier
...
# BronzeSourceConfig and BRONZE_SOURCES are now centralized in
# src.config.sources and src.config.sources.bronze_sources respectively.
# The adapter uses SourceRegistry to look up source configurations.
...
class BronzeScraperAdapter(BaseAdapter):
    ...
    def __init__(self, source_slug: str, *args: Any, **kwargs: Any) -> None:
        ...
        if config is None or not isinstance(config, BronzeSourceConfig):
            bronze_slugs = [
                s.slug for s in SourceRegistry.get_by_tier(SourceTier.BRONZE)
            ]
            raise ValueError(
                f"Unknown Bronze source: {source_slug}. "
                f"Available: {bronze_slugs}"
            )

        self.bronze_config = config
...
# Sources that have their own dedicated adapters in bronze/ subpackage
# should NOT be registered here (they register themselves).
_DEDICATED_ADAPTER_SLUGS = {
    # bronze/ subpackage adapters
    "larioja_agenda",
    "navarra_cultura",  # bronze/navarra.py
    "visitnavarra",
    "pamplona",
    "vacacionesseniors",
    "donarsangre",
...
for _source in SourceRegistry.get_by_tier(SourceTier.BRONZE):
    # Skip viralagenda sources (registered by bronze/viralagenda/base.py)
    if _source.slug.startswith("viralagenda_"):
        continue
    # Skip sources with dedicated adapters in bronze/ subpackage
    if _source.slug in _DEDICATED_ADAPTER_SLUGS:
        continue
    create_bronze_adapter_class(_source.slug)
